...
def progress_writer(percent_value):
...
# A limitek és bounce-back
    ...
    if checking_value >= 100:
        ...
        print("File disappeared error (code 7_FNF). Restarting upload from new directory...")
    ...
    progress_writer(percent_value)

# ASCII feltöltés bar
def upload_bar(percent_value):
    bar_length = 25
    filled = round(bar_length * (percent_value / 100))
    empty = bar_length - filled
    bar = "[" + "#" * filled + "-" * empty + "]" + " " + str(percent_value) + "%"
    return bar

print(upload_bar(percent_value))
